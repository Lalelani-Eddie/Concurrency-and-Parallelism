...
public class Grid {
    ...
    // This inner class will be used for parallel updates
    class UpdateTask extends Thread {
        private int startRow, endRow;

        public UpdateTask(int startRow, int endRow) {
    ...
    boolean update() {
        ...
        int rowsPerThread = (rows - 2) / numThreads;
        Thread[] threads = new Thread[numThreads];

        for (int t = 0; t < numThreads; t++) {
            int startRow = 1 + t * rowsPerThread;
            int endRow = (t == numThreads - 1) ? rows - 1 : startRow + rowsPerThread;
            threads[t] = new UpdateTask(startRow, endRow);
            threads[t].start();
        }
        ...
        // Check if there was any change and update the grid
        for (int i = 1; i < rows - 1; i++) {
            for (int j = 1; j < columns - 1; j++) {
